...
import HtmlSplitEditor from './components/HtmlSplitEditor.jsx'
...
const DEFAULT_HTML_BODY = '<div id="loading-screen">LOADING MODEL & TEXTURES. please wait...</div>\n\n<canvas id="webgl"></canvas>\n\n<div class="scroll-hint">Syntetika. Made by Curiosity. ↓</div>\n\n<section class="section-1">\n  <div class="content">\n    <h1>STK</h1>\n    <p>This is a test site for https://danartriyudistira.github.io/StkEditor/ . A code editor for VJ needs and people who don\'t fully understand livecoding.</p>\n  </div>\n</section>\n\n<section class="section-2">\n  <div class="content">\n    <h1>footage?</h1>\n    <p>Even though it can load images and videos, accessing them still requires code. The goal is to help VJs use interactive assets as footage.</p>\n  </div>\n</section>\n\n<section class="section-3">\n  <div class="content">\n    <h1>code Switch</h1>\n    <p>Switch between codes like a VJ plays videos, while still applying dynamic visual effects.</p>\n  </div>\n</section>'
const DEFAULT_HTML_CSS = ':root {\n  --bg-color-center: #16181d;\n  --bg-color-edge: #050507;\n  --text-primary: #ffffff;\n  --text-secondary: #a0a0b0;\n}\n* { margin: 0; padding: 0; box-sizing: border-box; }\nbody {\n  background: radial-gradient(circle at center, var(--bg-color-center) 0%, var(--bg-color-edge) 100%);\n  background-attachment: fixed;\n  color: var(--text-primary);\n  font-family: \'Helvetica Neue\', Arial, sans-serif;\n  overflow-x: hidden;\n  transition: background 0.3s ease-out, color 0.3s ease-out;\n}\nbody.inverted {\n  --bg-color-center: #ffffff;\n  --bg-color-edge: #e0e0e0;\n  --text-primary: #000000;\n  --text-secondary: #222222;\n}\n#webgl { position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; z-index: 1; pointer-events: none; }\n#loading-screen {\n  position: fixed; top: 0; left: 0; width: 100vw; height: 100vh;\n  background: #08080a; z-index: 10; display: flex; justify-content: center; align-items: center;\n  color: #00ffcc; font-size: 1.2rem; letter-spacing: 4px; transition: opacity 0.5s ease;\n}\nsection {\n  position: relative; height: 100vh; width: 100vw; display: flex;\n  flex-direction: column; justify-content: center; padding-left: 10%; z-index: 2;\n}\n.section-1 { justify-content: center; }\n.section-2 { align-items: flex-end; padding-right: 10%; padding-left: 0; }\n.section-3 { justify-content: center; }\n.content { padding: 20px; border-radius: 12px; transition: color 0.15s ease-out; }\nh1 {\n  font-size: 3.5rem; font-weight: 800; letter-spacing: 6px; text-transform: uppercase;\n  color: var(--text-primary); margin-bottom: 10px; transition: color 0.3s ease-out;\n}\np {\n  color: var(--text-secondary); font-size: 1.2rem; letter-spacing: 2px;\n  max-width: 400px; transition: color 0.3s ease-out;\n}\n.scroll-hint {\n  position: fixed; bottom: 30px; left: 50%; transform: translateX(-50%);\n  font-size: 0.8rem; letter-spacing: 3px; color: #00ffcc; z-index: 3;\n  text-transform: uppercase; opacity: 0.8; transition: color 0.3s ease-out;\n}\nbody.inverted .scroll-hint { color: #000000; }'
const DEFAULT_HTML_JS = 'function initApp() {\n  const canvas = document.querySelector(\'#webgl\');\n  const loadingScreen = document.querySelector(\'#loading-screen\');\n  if (!canvas || typeof THREE === \'undefined\' || typeof THREE.GLTFLoader === \'undefined\') return;\n\n  gsap.registerPlugin(ScrollTrigger);\n\n  const scene = new THREE.Scene();\n  const camera = new THREE.PerspectiveCamera(75, window.innerWidth / window.innerHeight, 0.1, 100);\n  camera.position.set(2, 0, 5);\n\n  const cameraTarget = new THREE.Vector3(2, 0, 0);\n\n  const renderer = new THREE.WebGLRenderer({ canvas: canvas, antialias: true, alpha: true });\n  renderer.setSize(window.innerWidth, window.innerHeight);\n  renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2));\n  renderer.outputEncoding = THREE.sRGBEncoding;\n\n  const ambientLight = new THREE.AmbientLight(0xffffff, 2.0);\n  scene.add(ambientLight);\n\n  const lightCyan = new THREE.PointLight(0x00ffcc, 5, 20);\n  const lightPurple = new THREE.PointLight(0x7928ca, 5, 20);\n  const lightWhite = new THREE.PointLight(0xffffff, 4, 20);\n  scene.add(lightCyan); scene.add(lightPurple); scene.add(lightWhite);\n\n  const dirLight = new THREE.DirectionalLight(0xffffff, 2.5);\n  dirLight.position.set(5, 10, 7);\n  scene.add(dirLight);\n\n  function createTextTexture(text) {\n    const c2 = document.createElement(\'canvas\'); c2.width = 1024; c2.height = 256;\n    const ctx = c2.getContext(\'2d\');\n    ctx.fillStyle = \'#08080a\'; ctx.fillRect(0, 0, 1024, 256);\n    ctx.font = \'Bold 90px Arial\'; ctx.fillStyle = \'#00ffcc\';\n    ctx.textAlign = \'center\'; ctx.fillText(text, 512, 160);\n    return new THREE.CanvasTexture(c2);\n  }\n\n  const textGeo = new THREE.CylinderGeometry(5, 5, 2, 32, 1, true);\n  const textRing1 = new THREE.Mesh(textGeo, new THREE.MeshBasicMaterial({\n    map: createTextTexture("STK EDITOR • LIVECODING VJ • "),\n    side: THREE.DoubleSide, transparent: true\n  }));\n  textRing1.position.set(0, -8, 0); scene.add(textRing1);\n\n  let obj1, obj2, obj3;\n\n  const loader = new THREE.GLTFLoader();\n  const modelURL = \'https://raw.githubusercontent.com/KhronosGroup/glTF-Sample-Models/master/2.0/DamagedHelmet/glTF-Binary/DamagedHelmet.glb\';\n\n  loader.load(modelURL, (gltf) => {\n    const baseModel = gltf.scene;\n    baseModel.traverse((child) => { if (child.isMesh) { child.castShadow = true; child.receiveShadow = true; } });\n    obj1 = baseModel.clone(); obj1.position.set(2, 0, 0); obj1.scale.set(3, 3, 3); scene.add(obj1);\n    obj2 = baseModel.clone(); obj2.position.set(-2, -8, 0); obj2.scale.set(3, 3, 3); scene.add(obj2);\n    obj3 = baseModel.clone(); obj3.position.set(0, -16, 0); obj3.rotation.set(0, -16, 0); obj3.scale.set(3, 3, 3); scene.add(obj3);\n    loadingScreen.style.opacity = \'0\'; setTimeout(() => loadingScreen.remove(), 500);\n    setupCameraAnimation();\n  });\n\n  const vs = "varying vec2 vUv;\\nvoid main() {\\n  vUv = uv;\\n  gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);\\n}";\n  const fs = "uniform sampler2D tDiffuse;\\nuniform float uProgress;\\nuniform float uTime;\\nvarying vec2 vUv;\\n\\nfloat random(vec2 st) {\\n  return fract(sin(dot(st.xy, vec2(12.9898,78.233))) * 43758.5453123);\\n}\\n\\nvoid main() {\\n  vec2 uv = vUv;\\n  float glitchLine = step(0.92, random(vec2(uTime * 20.0, floor(uv.y * 30.0))));\\n  uv.x += glitchLine * 0.03 * uProgress * (random(vec2(uTime)) - 0.5);\\n  float shift = 0.025 * uProgress;\\n  float r = texture2D(tDiffuse, uv + vec2(shift, 0.0)).r;\\n  float g = texture2D(tDiffuse, uv).g;\\n  float b = texture2D(tDiffuse, uv - vec2(shift, 0.0)).b;\\n  vec4 texColor = vec4(r, g, b, texture2D(tDiffuse, uv).a);\\n  vec3 color = texColor.rgb;\\n  float alpha = texColor.a;\\n  if (alpha > 0.1) { vec3 sc = vec3(0.02, 0.02, 0.03); color = mix(color, sc, uProgress * 0.95); }\\n  float strobe = step(0.5, sin(uTime * 50.0)) * uProgress;\\n  if (strobe > 0.8 && alpha > 0.1) color = vec3(1.0) - color;\\n  gl_FragColor = vec4(color, alpha);\\n}";\n\n  const InvertSilhouetteShader = {\n    uniforms: { "tDiffuse": { value: null }, "uProgress": { value: 0.0 }, "uTime": { value: 0.0 } },\n    vertexShader: vs,\n    fragmentShader: fs\n  };\n\n  const composer = new THREE.EffectComposer(renderer);\n  composer.addPass(new THREE.RenderPass(scene, camera));\n  const customPass = new THREE.ShaderPass(InvertSilhouetteShader);\n  customPass.renderToScreen = true; composer.addPass(customPass);\n\n  let scrollVelocity = 0, lastScrollY = window.scrollY;\n  window.addEventListener(\'scroll\', () => {\n    const dy = Math.abs(window.scrollY - lastScrollY);\n    scrollVelocity = Math.min(dy * 0.06, 1.0); lastScrollY = window.scrollY;\n  });\n\n  function setupCameraAnimation() {\n    const tl = gsap.timeline({ scrollTrigger: { trigger: "body", start: "top top", end: "bottom bottom", scrub: 1 } });\n    tl.to(camera.position, { x: -5, y: -6, z: 3, ease: "none" }, 0)\n      .to(cameraTarget, { x: -2, y: -8, z: 0, ease: "none" }, 0)\n      .to(camera.position, { x: 0, y: -15, z: -5, ease: "none" }, 1)\n      .to(cameraTarget, { x: 0, y: -16, z: 0, ease: "none" }, 1);\n  }\n\n  window.addEventListener(\'resize\', () => { camera.aspect = window.innerWidth / window.innerHeight; camera.updateProjectionMatrix(); renderer.setSize(window.innerWidth, window.innerHeight); composer.setSize(window.innerWidth, window.innerHeight); });\n\n  const clock = new THREE.Clock();\n  function animate() {\n    requestAnimationFrame(animate);\n    const t = clock.getElapsedTime();\n    camera.lookAt(cameraTarget);\n    const cy = camera.position.y;\n    lightCyan.position.set(camera.position.x + Math.sin(t * 0.8) * 4, cy + Math.sin(t * 0.5) * 2, camera.position.z + Math.cos(t * 0.8) * 4);\n    lightPurple.position.set(camera.position.x + Math.sin(t * 0.6 + 2) * 4, cy + Math.cos(t * 0.4) * 2, camera.position.z + Math.cos(t * 0.6 + 2) * 4);\n    lightWhite.position.set(camera.position.x + Math.cos(t * 1.2) * 3, cy + 2, camera.position.z + Math.sin(t * 1.2) * 3);\n    scrollVelocity *= 0.86; if (scrollVelocity < 0.001) scrollVelocity = 0;\n    if (scrollVelocity > 0.15) document.body.classList.add(\'inverted\');\n    else document.body.classList.remove(\'inverted\');\n    customPass.uniforms.uProgress.value = scrollVelocity;\n    customPass.uniforms.uTime.value = t;\n    textRing1.rotation.y = t * 0.3;\n    composer.render();\n  }\n  animate();\n}\n\nif (document.readyState === \'complete\') initApp();\nelse window.addEventListener(\'load\', initApp);'
function combineHtml(html, css, js) {
...
const DEFAULT_HTML = combineHtml(DEFAULT_HTML_BODY, DEFAULT_HTML_CSS, DEFAULT_HTML_JS)
...
export default function App() {
  ...
  const handleNewTab = useCallback((type) => {
    ...
    if (tabType === 'hydra') {
      ...
      newTab = { id, name: 'untitled.html', htmlCode: DEFAULT_HTML_BODY, cssCode: DEFAULT_HTML_CSS, jsCode: DEFAULT_HTML_JS, code: DEFAULT_HTML, type: 'html', modified: false }
    } else {
  ...
  const handleSwitchTabType = useCallback((type) => {
    setTabs(prev => prev.map(t => {
      ...
      if (type === 'hydra') {
        ...
        return { ...t, type, code: newCode, name: newName, htmlCode: DEFAULT_HTML_BODY, cssCode: DEFAULT_HTML_CSS, jsCode: DEFAULT_HTML_JS }
      } else {
  ...
  const handleNew = useCallback(() => {
    if (engineMode === 'hydra') {
      ...
      updateActiveTab({ code: DEFAULT_HTML, name: 'untitled.html', modified: false, htmlCode: DEFAULT_HTML_BODY, cssCode: DEFAULT_HTML_CSS, jsCode: DEFAULT_HTML_JS })
    } else {
